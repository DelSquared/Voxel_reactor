...
void sample_inter_dist(Neutron* n, float s[], int LEN_s){
	// this is kind of bad as it doesn't do anything to check if neutron interaction happens in same material
	// need to figure this out to check if distance to boundary is smaller than sample, then move to boundary and resample
	// thankfully the geometric distribution is memoryless
	float L = 1000000000;
	float sample;
	int reaction_type;
